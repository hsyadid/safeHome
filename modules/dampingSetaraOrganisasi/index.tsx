...
export const LayananHukum = () => {
  ...
  const fetchLawFirms = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:3001/law-firms');
      if (!response.ok) {
  ...
  const handleDeleteLawFirm = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus lembaga hukum ini?')) {
      try {
        const response = await fetch(`http://localhost:3001/law-firms/${id}`, {
          method: 'DELETE',
        });
  ...
  const handleSubmitLawFirm = async (data: any) => {
    try {
      ...
      if (editingLawFirm) {
        // Update law firm
        response = await fetch(`http://localhost:3001/law-firms/${editingLawFirm.id}`, {
          method: 'PATCH',
          headers: {
        ...
        // Add new law firm  
        response = await fetch('http://localhost:3001/law-firms', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
