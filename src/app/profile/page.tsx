...
export default function ProfilePage() {
  ...
  const fetchUserActivity = async () => {
    ...
    try {
      // Fetch user's posts
      const postsResponse = await fetch(`https://safehomeanara.id/api/posts/user/${user.userId}`);
      if (postsResponse.ok) {
      ...
      const repliesResponse = await fetch(`https://safehomeanara.id/api/replies/user/${user.userId}`);
